with report_config AS (
    SELECT
        ...
        gl_name
    FROM
        {{source('pink_taco', 'report_config')}}
),
...
entity_config AS (
    SELECT
        ...
        cc.concept_name
    FROM
       {{source('pink_taco', 'entity_config')}} en
    LEFT JOIN {{source('public', 'country_config_data')}} co
        ON en.country_id = co.country_id
    LEFT JOIN {{ source('pink_taco', 'concept_config') }} cc
...
source_trial_balance_manual AS (
    {% if adapter.get_relation(database=None, schema='pink_taco', identifier='trial_balances_manual') is not none %}
        SELECT
        ...
        FROM
            {{source('pink_taco', 'trial_balances_manual')}} tbm
...
target_trial_balance_manual AS (
    ...
    FROM(
        ...
        JOIN report_config rc 
            ON man.gl_number::text = rc.gl_number::text
        ...
        GROUP BY
            rc.company_id, ent.source_entity_id, ent.entity_name, ent.entity_id, ent.concept_id, ent.concept, ent.concept_name,
            ent.country_id, ent.country_name, rc.report_id, rc.report_name, rc.gl_number, rc.gl_name, rc.category_id,
            rc.category_name, rc.subcategory_id, rc.subcategory_name, rc.sub_subgroup_id, rc.sub_subgroup, rc.signage,
            rc.absolute_flag, mc_category.metric_order, mc_subcategory.submetric_order, mc_subgroup.subgroup_order, man.year,
            year_code, man.month, man.period, currency, period_net_dr, period_net_cr
    ) T1
),
...
target_trial_balance_manual_type AS (
    SELECT
        ...
        subcategory_name,
        gl_name,
        COALESCE(CAST(gl_number AS INTEGER), 0),
        sub_subgroup_id,
        sub_subgroup,
        beginning_balance,
        ending_balance,
