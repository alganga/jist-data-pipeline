WITH GUEST_CHECK_BASE AS (
    ...
    FROM(
        ...
        FROM {{source('xrg', 'guest_check')}} gc
        LEFT JOIN {{source('xrg', 'entity_config')}} entity
...
/*
    ...
    It uses the GUEST_CHECK_BASE table as the base data and joins it with the xrg_revenue_wtd table to retrieve the net sales values.
...
ACTUAL_WTD_MTD_YTD_CALCULATION AS (
    ...
    FROM(
        ...
        LEFT JOIN{{source('xrg', 'fiscal_calendar')}} calendar
            ON cast(t1.end_of_week AS VARCHAR) = cast(calendar.week_end_date AS VARCHAR)
        LEFT JOIN {{ref('xrg_revenue_wtd')}} revenue_wtd
