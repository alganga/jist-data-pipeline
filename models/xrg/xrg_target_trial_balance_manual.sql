...
entity_config AS (
    SELECT
        ...
        cc.concept_name
    FROM
       {{source('xrg', 'entity_config')}} en
    LEFT JOIN {{source('public', 'country_config_data')}} co
        ON en.country_id = co.country_id
    LEFT JOIN {{ source('public', 'state_config_data') }} st
        ON en.state_id = st.state_id
    LEFT JOIN {{ source('public', 'city_config_data') }} ci
        ON en.city_id = ci.city_id 
    LEFT JOIN {{ source('xrg', 'concept_config') }} cc
...
source_trial_balance_manual AS (
    {% if adapter.get_relation(database=None, schema='xrg', identifier='trial_balances_manual') is not none %}
        SELECT
        ...
        FROM
            {{source('xrg', 'trial_balances_manual')}} tbm
...
target_trial_balance_manual AS (
    ...
    FROM(
        ...
        GROUP BY
            rc.company_id, ent.source_entity_id, ent.entity_name, ent.entity_id, ent.city_id, ent.city_name, ent.state_id,
            ent.state_name, ent.country_id, ent.country_name, ent.concept_id, ent.concept, ent.concept_name,
