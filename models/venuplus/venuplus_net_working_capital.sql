...
aggregate_current_assets as (
    select 
        ...
        country_name,
        sum(ending_balance) as ending_balance
    from  {{ ref('venuplus_balance_sheet') }}
...
nwc_calculation as (
    select
        ...
        t1.country_id,
        t1.country_name,
        abs(current_assets) - abs(current_liabilties) as ending_balance
    from (
        select
            period,
            month,
            ...
            country_id,
            country_name,
            sum(case when category_name = 'Current Assets' then ending_balance else 0 end) as current_assets,
            sum(case when category_name = 'Current Liabilities' then ending_balance else 0 end) as current_liabilties
        from nwc
        group by 1, 2, 3, 4, 5, 6
    ) t1
)
