WITH profit_and_loss AS (
    select * from {{ ref('venuplus_profit_and_loss_staged') }}
),
metric_order AS (
...
mtd_ytd_ttm AS(
    SELECT
        ...
        COALESCE(p.budget,0) AS budget_mtd,
        p.ending_balance - COALESCE(y.year_opening_balance, 0) AS actual_ytd,
        SUM(COALESCE(p.ending_balance,0) - COALESCE(p.beginning_balance,0)) OVER (PARTITION BY p.entity_id, p.gl_number ORDER BY CAST(p.year AS INT), CAST(p.month AS INT) ROWS BETWEEN 11 PRECEDING AND CURRENT ROW) AS actual_ttm,
        SUM(p.budget) OVER (PARTITION BY p.entity_id, p.gl_number, p.year ORDER BY CAST(p.year AS INT), CAST(p.month AS INT)) AS budget_ytd,
        SUM(p.budget) OVER (PARTITION BY p.entity_id, p.gl_number ORDER BY CAST(p.year AS INT), CAST(p.month AS INT) ROWS BETWEEN 11 PRECEDING AND CURRENT ROW) AS budget_ttm
...
financial_aggregates AS(
    ...
    FROM mtd_ytd_ttm
    GROUP BY period, year, month, source_entity_id, entity_id, entity_name, country_id, country_name
),
...
LEFT JOIN mtd_ytd_ttm myt2
    ON myt.entity_id = myt2.entity_id
