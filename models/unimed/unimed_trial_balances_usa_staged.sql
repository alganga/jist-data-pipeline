 WITH DistinctGls AS (
    SELECT DISTINCT
        gl_code_id
    FROM {{ source('unimed', 'trial_balances_usa') }} 
),
...
AllDatesAndGLs AS (
    SELECT 
        ...
        ref_date
    FROM DistinctGls
    CROSS JOIN (SELECT DISTINCT ref_date FROM {{ source('unimed', 'trial_balances_usa') }} ) AS GLs
),
...
BaseData AS (
    SELECT 
        ...
        C.country_number,
        C.country_name
    FROM {{ source('unimed', 'trial_balances_usa') }} C
...
JoinData AS (
    ...
    FROM BaseData T1
    JOIN {{ source('unimed', 'trial_balances_usa') }} T2
