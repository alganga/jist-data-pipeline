...
entity_config AS (
    SELECT
        ...
        co.country_name
    FROM
       {{source('unimed', 'entity_config_data')}} en
...
source_trial_balance_manual AS (
    {% if adapter.get_relation(database=None, schema='unimed', identifier='trial_balances_manual') is not none %}
        SELECT
        ...
        FROM
            {{source('unimed', 'trial_balances_manual')}} tbm
...
target_trial_balance_manual_type AS (
    SELECT
        company_id,
        to_date(year || '-' || month, 'YYYY-MM') AS date,
        country_id,
        country_name,
        report_id,
        report_name,
        ...
        sub_subgroup_id,
        sub_subgroup,
        period_net_dr as debit,
        period_net_cr as credit,
        ending_balance,
        metric_order,
        submetric_order,
        subgroup_order,
